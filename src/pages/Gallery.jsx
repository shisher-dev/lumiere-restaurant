...
function Gallery() {
  ...
  return (
    <main className="bg-[#101010] px-5 py-24 text-white sm:px-8 lg:px-10">
      <div className="mx-auto max-w-7xl">
        ...
        {loading ? (
          <div className="mt-14 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="h-105 animate-pulse rounded-2xl bg-white/5 sm:col-span-2 lg:row-span-2" />

            {[1, 2, 3, 4].map((item) => (
              <div
                key={item}
                className="h-75 animate-pulse rounded-2xl bg-white/5"
              />
            ))}
          </div>
        ) : (
                  ...
                  {!isLarge && (
                    <div className="absolute bottom-4 left-4 rounded-full border border-white/20 bg-black/40 px-4 py-2 backdrop-blur-md">
                      <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-white">
                        {item.label || item.category || "Lumière"}
                      </span>
                    </div>
                  )}
                </article>
